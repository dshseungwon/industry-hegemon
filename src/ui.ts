...
let sheetMoreOpen = false;   // 국가 시트 '자세히' 펼침 상태(매 틱 재렌더에도 유지)
...
function renderSheet(s: GameState, A: Actions) {
  ...
  el.innerHTML = '<button class="x" id="closeSheet">✕</button><h3>' + m.ko + ' <span class="mute small">' + m.name + '</span></h3>' +
    ...
    allocSect(s) +
    lobbyBtn(s) +
    '<details class="more"' + (sheetMoreOpen ? ' open' : '') + '><summary><span class="chev">▾</span> 자세히 — 점유율·적합도·영향력</summary>' +
      ...
      fitDiagnosis(s) +
      infBars(s, s.ui.country) +
    '</details>';
  document.getElementById("closeSheet")!.onclick = () => A.selectCountry(null);
  const dmore = el.querySelector("details.more") as HTMLDetailsElement | null;
  if (dmore) dmore.addEventListener("toggle", () => { sheetMoreOpen = dmore.open; });
